...
3. **`bash ralph.sh`**: Fully automated loop through remaining tasks until all complete
...
This is a project-level command placed in each project's `.claude/commands/` directory.
...
### 4. Ralph Bash Script `ralph.sh` (project root template)
...
while [ $ITERATION -lt $MAX_ITERATIONS ]; do
  ...
  OUTPUT=$(claude -p "
    Read feature-requirements.md and progress.txt.
...
- Run `chmod +x ralph.sh` before executing the script
- Global `CLAUDE.md` applies to all projects; `.claude/commands/` is project-specific
- Add `progress.txt` and `feature-requirements.md` to `.gitignore`
- Do not interrupt the Ralph loop while it's running — review commit history after it finishes
- To restore this setup on a new machine: clone this repo and re-run this instruction with Claude Code
